...
with tf.device('/gpu:0'):
	...
	Z_real_encoded, _, _ = Sampler(X_real_encoded)
	X_real_encoded_reshaped = tf.reshape(Z_real_encoded, [BATCH_SIZE, UNFOLD_SIZE, CELL_SIZE])
...
for iteration1 in range(ITR) :
	...
	if iteration1%2500 == 0 :
		...
		for j in range(32) :
			...
			recn___ = sess.run(recn, feed_dict = { GEN_pred_0 : get_noise })
