...
class dataLoader :
	...
	def GetNextBatch(self, batch_size = BATCH_SIZE): 
		...
		for a_batch in range(BATCH_SIZE):
			...
			# Here is the BUG!! The listing is alphabetical! 
			# im_list = os.listdir(this_dir)
			# print '[DEBUG] ', im_list
			# print '[DEBUG] ', im_list.shape
			# print '[DEBUG] Exiting : '
			# sys.exit()
			...
			# Add videos
			for _ in range(5):
				next_im = cv2.imread(this_dir + '/' + str(first) + '.png', cv2.IMREAD_GRAYSCALE)
				print '[DEBUG] Name of the selected image : ', str(this_dir + '/' + str(first) + '.png')
				# print '[DEBUG] 'this_dir + '/' + im_list[first] ## This thing showed the BUG
				# print '[DEBUG]', next_im.shape
				# print '[DEBUG]', next_im
				# print '[DEBUG]', np.where(next_im != 0)
				# print '[DEBUG] Exiting ... '
				# sys.exit()
...
print np.linalg.norm(next_batch)
