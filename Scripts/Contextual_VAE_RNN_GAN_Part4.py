...
with tf.device('/gpu:0'):
	...
	# Placeholdersi
	X_im = tf.placeholder(tf.float32, [BATCH_SIZE, UNFOLD_SIZE, IMG_SIZE, IMG_SIZE, ENC_INPUT_CHANNEL], name = 'ph1')
	X_im_reshaped = tf.reshape(X_im, [BATCH_SIZE*UNFOLD_SIZE, IMG_SIZE, IMG_SIZE, ENC_INPUT_CHANNEL])
	ENC_trial_ph = tf.placeholder(tf.float32, [BATCH_SIZE*UNFOLD_SIZE, IMG_SIZE, IMG_SIZE, ENC_INPUT_CHANNEL], name = 'ph2')
	DEC_trial_ph = tf.placeholder(tf.float32, [BATCH_SIZE*UNFOLD_SIZE, IMG_SIZE/16, IMG_SIZE/16, 128], name = 'ph3')
	# Encoder Variables
	...
	# Sampler variables
	SAM_W_mu = tf.Variable(tf.truncated_normal([CELL_SIZE, CELL_SIZE], stddev = 0.1), name = 'SAM_W_mu')
	SAM_B_mu = tf.Variable(tf.zeros([CELL_SIZE]), name = 'SAM_B_mu')
	SAM_W_logstd = tf.Variable(tf.truncated_normal([CELL_SIZE, CELL_SIZE], stddev = 0.1), name = 'SAM_W_logstd')
	SAM_B_logstd = tf.Variable(tf.zeros([CELL_SIZE]), name = 'SAM_B_logstd')
...
with tf.device('/gpu:0'):
	# Definition of the Sampler Forward Pass
	def Sampler(x):
		x_ = tf.reshape(x, [-1, CELL_SIZE])	
		SAM_mu = tf.add(tf.matmul(x_, SAM_W_mu), SAM_B_mu)
		SAM_logstd = tf.add(tf.matmul(x_, SAM_W_logstd), SAM_B_logstd)
		SAM_noise = tf.random_normal([1, CELL_SIZE])
		SAM_Z = SAM_mu + tf.multiply(SAM_noise, tf.exp(0.5*SAM_logstd))
		SAM_Z_ = tf.reshape(SAM_Z, [-1, IMG_SIZE/16, IMG_SIZE/16, 128])
		return SAM_Z_, SAM_mu, SAM_logstd
...
soft_1_label = tf.random_uniform(shape = [BATCH_SIZE, 1], minval = 0.9, maxval = 1.1)
...
vid_encoding = tf.reshape(GEN_pred_vid_5, [BATCH_SIZE*UNFOLD_SIZE, CELL_SIZE])
vid_encoding_reshaped, vid_encoding_reshaped_mean, _ = Sampler(vid_encoding)
# vid_encoding_reshaped = tf.reshape(vid_encoding, [BATCH_SIZE*UNFOLD_SIZE, IMG_SIZE/16, IMG_SIZE/16, 128])
recn_reshaped = DecoderPix2Pix(vid_encoding_reshaped, True)
recn_reshaped_mean = DecoderPix2Pix(vid_encoding_reshaped_mean, True)
recn = tf.reshape(recn_reshaped, [BATCH_SIZE, UNFOLD_SIZE, IMG_SIZE, IMG_SIZE, ENC_INPUT_CHANNEL])
recn_mean = tf.reshape(recn_reshaped_mean, [BATCH_SIZE, UNFOLD_SIZE, IMG_SIZE, IMG_SIZE, ENC_INPUT_CHANNEL])
with tf.device('/gpu:0'):
	...
	# Define D_of_X
	X_real = tf.placeholder(tf.float32, [BATCH_SIZE, UNFOLD_SIZE, IMG_SIZE, IMG_SIZE, ENC_INPUT_CHANNEL], name = 'ph7')
	X_real_reshaped = tf.reshape(X_real, [BATCH_SIZE*UNFOLD_SIZE, IMG_SIZE, IMG_SIZE, ENC_INPUT_CHANNEL])
...
# Define for VAE
	...
	Z_enc, mu_enc, logstd_enc = Sampler(H_enc)
	X_rec_reshaped = DecoderPix2Pix(Z_enc, True)
	...
	KLT = -0.5*tf.reduce_sum(1 + 2*logstd_enc - tf.pow(mu_enc, 2) - tf.exp(2*logstd_enc), axis = 1)
...
with tf.device('/gpu:0'):
    training_step_optimizer = tf.train.AdamOptimizer(learning_rate=0.001, beta1=0.9, beta2=0.999, epsilon=1e-08, name = 'Adam1')
    #rning_rate=0.001, L2_loss = tf.nn.l2_loss(X_hat - X)
    # L2_training_step = tf.train.AdamOptimizer().minimize(L2_loss)
    DIS_fake_training_step_optimizer = tf.train.AdamOptimizer(learning_rate=0.001, beta1=0.9, beta2=0.999, epsilon=1e-08, name = 'Adam2')
    DIS_real_training_step_optimizer = tf.train.AdamOptimizer(learning_rate=0.001, beta1=0.9, beta2=0.999, epsilon=1e-08, name = 'Adam3')
    GEN_training_step_optimizer = tf.train.AdamOptimizer(learning_rate=0.001, beta1=0.9, beta2=0.999, epsilon=1e-08, name = 'Adam4')


#with tf.device('/gpu:0'):
#    training_step = training_step_optimizer.minimize(-VLB, var_list = ENC_trainable_variables_list + DEC_trainable_variables_list)
#    # L2_loss = tf.nn.l2_loss(X_hat - X)
#    # L2_training_step = tf.train.AdamOptimizer().minimize(L2_loss)
#    DIS_fake_training_step = DIS_fake_training_step_optimizer.minimize(DIS_fake_loss, var_list = GEN_trainable_variables_list + DIS_trainable_variables_list)
#    DIS_real_training_step = DIS_real_training_step_optimizer.minimize(DIS_real_loss, var_list = GEN_trainable_variables_list + DIS_trainable_variables_list)
#    GEN_training_step = GEN_training_step_optimizer.minimize(GEN_loss, var_list = GEN_trainable_variables_list + DIS_trainable_variables_list)
...
for item in DIS_trainable_variables_list :
	print('[DEBUG] 		' + str(item.name))
SAM_trainable_variables_list = [item for item in trainable_variables_list if 'SAM' in item.name]
print('[DEBUG] SAM Trainable Variables List : ' + str(len(SAM_trainable_variables_list)))
for item in SAM_trainable_variables_list :
	print('[DEBUG] 		' + str(item.name))
...
with tf.device('/gpu:0'):
	training_step = training_step_optimizer.minimize(-VLB, var_list = ENC_trainable_variables_list + DEC_trainable_variables_list + SAM_trainable_variables_list)
#	# L2_loss = tf.nn.l2_loss(X_hat - X)
#	# L2_training_step = tf.train.AdamOptimizer().minimize(L2_loss)
	DIS_fake_training_step = DIS_fake_training_step_optimizer.minimize(DIS_fake_loss, var_list = GEN_trainable_variables_list + DIS_trainable_variables_list)
	DIS_real_training_step = DIS_real_training_step_optimizer.minimize(DIS_real_loss, var_list = GEN_trainable_variables_list + DIS_trainable_variables_list)
...
for iteration in range(VAE_ITR) : 
	...
	sess.run(training_step, feed_dict = { X_im : training_batch })
	if iteration%500 == 0 :
		print('[TRAINING] Iteration : ' + str(iteration))
...
for iteration1 in range(ITR) :
	print('[TRAINING] Iteration : ' + str(iteration1))
	training_batch = data_loader.GetNextBatch()
	sess.run(DIS_real_training_step, feed_dict = { X_real : training_batch })
	get_noise = sess.run(noise)
	sess.run(DIS_fake_training_step, feed_dict = { GEN_pred_0 : get_noise })
	get_noise = sess.run(noise)
	sess.run(GEN_training_step, feed_dict = { GEN_pred_0 : get_noise })
	if iteration1%100 == 0 :
		print('[TRAINING] Iteration : ' + str(iteration1))
		# if iteration1%500 == 0 :
		get_noise = sess.run(noise)
	...
	if iteration1%5000 == 0 :
		...
		for j in range(1) :
			...
			recn___, recn___mean = sess.run([recn, recn_mean], feed_dict = { GEN_pred_0 : get_noise })
			data[-1].append(recn___)
			data[-1].append(recn___mean)
		data_np = np.array(data)
		np.save('C_RNN_GAN_Generated_Dataset/Iteration_' + str(iteration1), data_np)
		os.system('mkdir C_RNN_GAN_Models/Iteration_' + str(iteration1))
		saver.save(sess, 'C_RNN_GAN_Models/Iteration_' + str(iteration1) + '/Intermediate_Model_' + str(iteration1))
