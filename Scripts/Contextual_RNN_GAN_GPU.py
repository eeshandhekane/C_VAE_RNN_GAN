...
GEN_pred_00 = tf.placeholder(dtype = tf.float32, shape = [BATCH_SIZE, 1, LATENT_DIM], name = 'GEN_pred_00')
GEN_h_0 = tf.zeros([BATCH_SIZE, CELL_SIZE*STACK_SIZE])
#print '[DEBUG] ################################################################################'
#print '[DEBUG] ################################################################################'
# This is mainly for generating images one-by-one
[GEN_pred_1, GEN_h_1] = Generator(GEN_pred_00, GEN_h_0)
...
for iteration in range(ITR):
	...
	if iteration%2000 == 0:
		...
		noise = sess.run(GEN_pred_0)
		print('[DEBUG] Noise vector : ' + str(noise) + ' Shape of noise : ', str(noise.shape))
		#print('[DEBUG] Exiting the code : ')
		#sys.exit()
		[im1, im2, im3, im4, im5, im6] = sess.run([reconstr_1, reconstr_2, reconstr_3, reconstr_4, reconstr_5, reconstr_6], feed_dict = {GEN_pred_00 : noise})
